...
add_filter(
	...
	function ( $text ) {
		...
		if ( $screen && 'toplevel_page_dc-script-worker-prefetcher' === $screen->id ) {
			return sprintf(
...
function dc_swp_enqueue_admin_assets( $hook ) {
	if ( 'toplevel_page_dc-script-worker-prefetcher' !== $hook ) {
		return;
	}
    ...
    /* CSS-only badge -- pure CSS pill rendered from data-label / data-msg. */
...
/**
 * Sanitize a Script Center JS body entered by an administrator.
 *
 * The Script Center accepts the *body* of a JavaScript snippet (no surrounding
 * <script> tags) and stores it as inert text. At render time it is wrapped in
 * <script type="text/plain" data-wp-consent-category="...">...</script> -- the
 * browser never executes it until a consent layer (WP Consent API / Partytown)
 * flips the type to text/javascript or text/partytown. The same approach is
 * used by the Complianz Script Center.
 *
 * sanitize_text_field() strips < and >, collapses newlines and removes NUL
 * bytes, which prevents the value from breaking out of its <script> wrapper
...
/**
 * Sanitize callback for the dc_swp_inline_scripts option (Script Center).
 *
 * Stored as a JSON-encoded array of script block objects:
 *   { id, label, code, src, async, enabled, skip_logged_in, category }
 *
 *   - id, label : identifiers / display label
 *   - code      : JS body sanitized via dc_swp_sanitize_js_code()
 *                 (sanitize_text_field; output as <script type="text/plain">)
 *   - src       : optional external script URL sanitized via esc_url_raw()
 *   - async     : boolean toggle for the async attribute on the external src
 *   - category  : WP Consent API category (allow-listed)
 *
...
function dc_swp_admin_page_html() {
	...
	if ( isset( $_POST['dc_swp_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['dc_swp_nonce'] ) ), 'dc_swp_save_settings' ) ) {
		...
		// Script Center: decode the JS-managed JSON accordion payload and run
		// it through the same sanitiser used by the option's register_setting()
		// callback. dc_swp_sanitize_inline_scripts_option() applies
		// sanitize_text_field() to the JS body (Complianz pattern) and
		// esc_url_raw() to the optional external src.
		$raw_json_blocks = wp_unslash( $_POST['dc_swp_inline_scripts_json'] ?? '' ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- JSON envelope; each field sanitized inside dc_swp_sanitize_inline_scripts_option().
		update_option( 'dc_swp_inline_scripts', dc_swp_sanitize_inline_scripts_option( $raw_json_blocks ) );
					...
					<details class="dc-swp-consent-info">
						...
						<div class="dc-swp-consent-info-body">
							...
							<div class="dc-swp-badges">
								<?php
								foreach ( $_gcm as $_b ) {
									echo $_badge( $_b[0], $_b[1], $_b[2] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- badge HTML built with esc_attr inside helper.
								}
								?>
							</div>
